document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadUserEvents() {
        ...
        // Changed to save as a flat array
    ...
    function openModal(data = {}) {
        ...
        if (event) { // Editing an existing event
            ...
            // Use startDate for recurring events
    ...
    function updateHeaderDate() {
        ...
        document.getElementById('current-date').textContent = displayedDate.toLocaleDateString('en-US', {
            weekday: 'long',
            month: 'long',
            day: 'numeric'
        });
    ...
    function renderEvents() {
        ...
        // Check for user-created recurring events
        userEvents.forEach(event => {
            ...
            if (!recurrence || recurrence.type === 'none') {
                ...
                if (recurrence.days.includes(displayedDate.getDay())) {
                    eventsForDay.push(event);
                }
            }
        });
        ...
        const processedEvents = eventsForDay.map(event => ({
            ...event,
            startMinutes: parseInt(event.start.split(':')[0]) * 60 + parseInt(event.start.split(':')[1]),
            endMinutes: parseInt(event.end.split(':')[0]) * 60 + parseInt(event.end.split(':')[1]),
        ...
        processedEvents.forEach(event => {
            ...
            let currentCluster = [];
            processedEvents.forEach(otherEvent => {
                 if (!otherEvent.placed && Math.max(event.startMinutes, otherEvent.startMinutes) < Math.min(event.endMinutes, otherEvent.endMinutes)) {
                    currentCluster.push(otherEvent);
                 }
            });
            if(currentCluster.length > 0) {
                 currentCluster.forEach(e => e.placed = true);
    ...
    function updateTimeIndicator() {
        const now = new Date();
        if (getYYYYMMDD(displayedDate) !== getYYYYMMDD(now)) {
    ...
    function handleFormSubmit(e) {
        ...
        if (recurrence.type === 'weekly') {
            recurrence.days = Array.from(document.querySelectorAll('#weekly-recurrence-options input:checked')).map(cb => parseInt(cb.value));
        }

        const newEvent = {
            id,
            title: eventTitleInput.value,
            startDate: eventDateInput.value, // This is the first day the event occurs
            start: eventStartTimeInput.value,
            end: eventEndTimeInput.value,
            colorClass: eventColorInput.value,
            recurrence
        };

        let userEvents = loadUserEvents();
        const existingIndex = userEvents.findIndex(e => e.id === id);
